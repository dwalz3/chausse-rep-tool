...
function KpiCard({ label, value, sub }: { label: string; value: string | number; sub?: string }) {
  return (
    <div style={{ backgroundColor: '#FFFFFF', borderRadius: 10, border: '1px solid #E5E1DC', padding: '18px 20px', flex: 1, minWidth: 0 }}>
      <p style={{ margin: 0, fontSize: 12, color: '#a8a29e', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</p>
      <p style={{ margin: '6px 0 2px', fontSize: 26, fontWeight: 700, color: '#1C1917' }}>{value}</p>
  ...
  return (
    <Shell>
      <div style={{ maxWidth: 960 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1C1917', margin: '0 0 20px' }}>Dashboard</h1>

        {!hasData && (
          <div style={{ backgroundColor: '#FFFFFF', borderRadius: 10, border: '1px solid #E5E1DC', padding: '32px', textAlign: 'center', color: '#a8a29e', fontSize: 14 }}>
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
              ...
              {goalIsSet ? (
                ...
                <div
                  style={{ backgroundColor: '#FFFFFF', borderRadius: 10, border: '1px dashed #E5E1DC', padding: '18px 20px', flex: 1, minWidth: 0, cursor: 'pointer' }}
                  onClick={() => router.push('/settings')}
                >
                  <p style={{ margin: 0, fontSize: 12, color: '#a8a29e', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Goal Attainment</p>
                  <p style={{ margin: '6px 0 0', fontSize: 13, color: '#a8a29e' }}>Set a monthly goal in <span style={{ color: '#2D5A3D', fontWeight: 600 }}>Settings →</span></p>
                </div>
              )}
            ...
            <div style={{ backgroundColor: '#FFFFFF', borderRadius: 10, border: '1px solid #E5E1DC', padding: '16px 20px', marginBottom: 16 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: '#1C1917', margin: '0 0 12px' }}>Revenue Trend (13 months)</h3>
              <RepRevenueChart rc5Data={rc5Data} rep={rep!} />
            </div>

            {/* Priority Actions */}
            <h2 style={{ fontSize: 16, fontWeight: 700, color: '#1C1917', margin: '0 0 12px' }}>Today&apos;s Priorities</h2>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              {/* Dormant card */}
              {dormantCount > 0 && (
                <div
                  onClick={() => router.push('/dormant')}
                  style={{ flex: 1, minWidth: 200, backgroundColor: '#FFFFFF', borderRadius: 10, border: '1px solid #FEF3C7', padding: '16px 18px', cursor: 'pointer' }}
                  onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = '#FFFBEB')}
                  onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = '#FFFFFF')}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <AlertCircle size={16} color="#d97706" />
                    <span style={{ fontSize: 13, fontWeight: 700, color: '#1C1917' }}>Dormant Accounts</span>
                  </div>
                  <p style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#d97706' }}>{dormantCount}</p>
                  <p style={{ margin: '4px 0 0', fontSize: 12, color: '#a8a29e' }}>accounts need re-engagement →</p>
                </div>
              )}
              ...
              {atRiskCount > 0 && (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <TrendingDown size={16} color="#b45309" />
                    <span style={{ fontSize: 13, fontWeight: 700, color: '#1C1917' }}>At-Risk Accounts</span>
                  </div>
                  <p style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#b45309' }}>{atRiskCount}</p>
                  <p style={{ margin: '4px 0 0', fontSize: 12, color: '#a8a29e' }}>dropping in last month →</p>
                </div>
              )}
              ...
              <div
                ...
                style={{ flex: 1, minWidth: 200, backgroundColor: '#FFFFFF', borderRadius: 10, border: '1px solid #D1FAE5', padding: '16px 18px', cursor: 'pointer' }}
                onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = '#ECFDF5')}
                onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = '#FFFFFF')}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <TrendingUp size={16} color="#16a34a" />
                  <span style={{ fontSize: 13, fontWeight: 700, color: '#1C1917' }}>Focus List</span>
                </div>
                <p style={{ margin: 0, fontSize: 13, color: '#16a34a', fontWeight: 600 }}>Top performers</p>
                <p style={{ margin: '4px 0 0', fontSize: 12, color: '#a8a29e' }}>view your top wines to push →</p>
              </div>
            </div>
          </>
        )}
